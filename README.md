...
### 2026-08-09（通用长篇小说自动化主线升级）

这次更新把产品入口从“功能集合”进一步收拢为通用长篇小说自动化主线：先选类型模板和创作目标，再沿着自动导演、单书工作台、章节执行、审校修复和状态回灌继续推进。

- 首页改为更直接的长篇主线驾驶舱，突出“开始一部长篇小说”“继续最近的作品”和导演跟进入口，让新手先看到下一步。
- 新增通用长篇模板注册表与创建页模板引导，支持男频、女频、悬疑、科幻、奇幻、现实等方向的默认问题和规划重点，同时保留自定义模板，不把项目锁死在单一题材。
- 单书工作台新增主线状态面板，解释当前自动导演阶段、暂停原因、推荐动作和可继续路径。
- 导演跟进详情加入用户可读的风险和恢复解释，不只展示任务字段。
- 章节编辑器新增生产闭环状态卡，展示任务书、正文、审校、修复和状态同步是否就绪，帮助判断本章有没有真正进入长篇连续资产。
- 前端字体、按钮、卡片、模板选择和主线状态面板补充更克制的过渡动效与纸面控制台质感，降低默认后台模板感。
- 顶部模型选择器、输入框、下拉框和首页首屏进一步收紧，减少大白表单块和过度留白，更像持续使用的生产控制台。
- README、贡献说明、PR 模板和公开文档补充 AI 辅助修改、第三方内容来源披露和宫寒个人版维护边界。
