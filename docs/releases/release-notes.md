...
这次更新把宫寒个人版的核心体验从“功能入口集合”收拢为通用长篇小说自动化主线。用户不需要先理解所有模块，而是可以从首页、创建页、单书工作台、导演跟进和章节编辑器一路看到当前阶段、下一步动作和质量闭环状态。

- 首页改为长篇主线驾驶舱，突出“开始一部长篇小说”“继续最近的作品”、导演跟进和任务中心入口，让新手先从目标进入。
- 新增长篇模板注册表，提供男频、女频、悬疑、科幻、奇幻、现实等通用题材方向的默认问题、规划重点、审稿关注点和自动导演建议，同时保留自定义模板。
- 小说创建页加入模板引导，能把模板默认问题和创作重点带入简介、卖点与前 30 章承诺；切回自定义时会清理模板生成的占位指导，避免误保留旧题材。
- 单书工作台新增主线状态面板，用用户能理解的语言说明自动导演阶段、暂停原因、推荐动作、风险等级和可继续路径。
- 移动端单书工作台同步显示主线状态，避免手机上只看到任务入口却不知道下一步该做什么。
- 导演跟进详情接入工作流解释，展示阶段、暂停原因、建议动作、风险提示和是否可继续，保留原有安全修复、重新校验和执行动作。
- 章节编辑器新增生产闭环状态卡，展示任务书、正文、审校、修复和状态同步是否就绪，帮助判断本章是否已经进入可连续推进的长篇资产。
- 前端视觉系统进一步统一为“出版编辑部 + 导演控制台”方向，优化中文字体栈、按钮 hover/active 状态、卡片阴影、模板选择卡片、主线状态面板和首页入口过渡，并保留降低动态偏好适配。
- 顶部栏高度、模型选择器、输入框、下拉框和首页首屏密度继续收紧，减少大白表单块、粗焦点环和过度留白，让主界面更接近生产调度台。
- 补充贡献说明、PR 模板和公开文档导航，明确 AI 辅助修改、第三方代码/素材/数据/模型输出等内容需要披露来源、许可证和必要归属信息。
- 新增长篇自动化设计规格与执行计划文档，后续维护可以继续沿着通用长篇主线推进，而不是回到单一题材或零散工具优化。
